...
func (service) GetServerName(c io.Reader, buf []byte) (uint, []byte, error) {
	...
	if cap(mbuf) < int(length) {
		return uint(n), nil, io.ErrShortBuffer
	}
	...
	if r.ReadUint8() != 1 {
		return uint(n), nil, ErrNoClientHello
	}

	l := r.ReadUint24()
	if l != uint32(length)-4 {
		return uint(n), nil, errors.WithContext("error reading body: ", ErrInvalidLength)
	}
	...
	if sessionLength > 32 || len(mbuf) < int(sessionLength) {
		// invalid length
		return uint(n), nil, errors.WithContext("error reading sesion id: ", ErrInvalidLength)
	}
	...
	if cipherSuiteLength == 0 || len(mbuf) < int(cipherSuiteLength) {
		// invalid length
		return uint(n), nil, errors.WithContext("error reading cipher suites: ", ErrInvalidLength)
	}
	...
	if compressionMethodLength < 1 || len(mbuf) < int(compressionMethodLength) {
		return uint(n), nil, errors.WithContext("error reading compressions: ", ErrInvalidLength)
	}
	mbuf = mbuf[compressionMethodLength:] // skip compression methods

	extsLength := r.ReadUint16()
	if len(mbuf) < int(extsLength) {
		return uint(n), nil, errors.WithContext("error reading extensions: ", ErrInvalidLength)
	}
	...
	for len(mbuf) > 0 {
		...
		if len(mbuf) < int(extLength) {
			return uint(n), nil, errors.WithContext("error reading extension: ", ErrInvalidLength)
		}
		if extType == 0 { // server_name
			l := r.ReadUint16()
			if l != extLength-2 {
				return uint(n), nil, errors.WithContext("error reading server name extension: ", ErrInvalidLength)
			}
			...
			if len(mbuf) < int(nameLength) {
				return uint(n), nil, errors.WithContext("error reading server name: ", ErrInvalidLength)
			}
			return uint(n), mbuf[:nameLength], nil
		} else {
			mbuf = mbuf[extLength:]
		}
	}
	return uint(n), nil, ErrNoName
}
